...
class CourseDownloadUpdaterTest extends \Youkok2\Tests\YoukokTestCase
{
    ...
    public function testCourseDownloadUpdater() {
        $course1 = new Element();
        $course1->setParent(null);
        $course1->setName('Course 1');
        $course1->setPending(false);
        $course1->setDeleted(false);
        $course1->save();

        $course2 = new Element();
        $course2->setParent(null);
        $course2->setName('Course 2');
        $course2->setPending(false);
        $course2->setDeleted(false);
        $course2->save();

        $individual_downloads = [
            1, 5, 4, 22, 15, 1, 2, 7, 3, 23, 11, 5, 44, 7, 8, 15, 3, 6, 2, 1, 7, 10, 12, 4, 6
        ];

        for ($i = 1; $i <= 25; $i++) {
            $element = new Element();
            $element->setName('Element ' . $i);
            $element->setParent($course1->getId());
            $element->setPending(false);
            $element->save();

            for ($j = 0; $j < $individual_downloads[$i - 1]; $j++) {
        ...
        $this->assertEquals(44, $course_downloads_always_data[0]['downloaded']);
        ...
        $this->assertEquals(5, $course_downloads_today_data[0]['downloaded']);
        ...
        $this->assertEquals(22, $course_downloads_week_data[0]['downloaded']);
        ...
        $this->assertEquals(23, $course_downloads_month_data[0]['downloaded']);
        ...
        $this->assertEquals(44, $course_downloads_year_data[0]['downloaded']);
    }
}
