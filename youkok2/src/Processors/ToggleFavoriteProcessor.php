...
class ToggleFavoriteProcessor
{
    ...
    private static function addFavorite(Element $element, SessionHandler $sessionHandler)
    {
        ...
        $sessionHandler->setData('favorites', $newFavorites, SessionHandler::MODE_OVERWRITE);
    ...
    private static function removeFavorite(Element $element, SessionHandler $sessionHandler)
    {
        $favorites = $sessionHandler->getDataWithKey('favorites');
        if (!in_array($element->id, $favorites)) {
            return false;
