...
class ElementController {
    ...
    public function createById($id) {
        ...
        if ($this->cache and CacheManager::isCached($id, 'i') and 1==2) {
    ...
    public function hasCourse() {
        echo gettype($this->model->getCourse());
    ...
    private function cacheFormat() {
        ...
        foreach ($fields as $v) {
            ...
            if (method_exists('\Youkok2\Models\Element', $v)) {
                var_dump( $this->hasCourse());
                if ($v == 'getCourse' ) {
                    echo 'her';
                    $cache_temp[] = "'" . $v_pretty . "' => '" . addslashes(call_user_func(array($this->model->getCourse()->getId(), $v))) . "'";
                }
                else {
    ...
    public function getFrontpageLink($mode = null, $special = null) {
        ...
        if ($this->model->isDirectory()) {
            ...
            if ($root_parent != null) {
                ...
                $ret .= ' data-placement="top" title="x">' . $root_parent->getName() . '</a>' . PHP_EOL;
            }
        }
