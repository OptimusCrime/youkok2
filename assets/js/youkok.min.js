    ...
    $('#archive-create-link-form').on('submit', function () {
        ...
        if (link_name.length == 0) {
            ...
            if (!check_url(link_name)) {
                display_message([{'text': '\''+link_name+' \' er ikke en gyldig URL.','type':'danger'}])}else{// Update queue;if(!submitting_archive_create_link_form){submitting_archive_create_link_form=true;// Update working;$('#archive-create-link-form-submit').html('Jobber...').prop('disabled',true);$.ajax({cache:false,type:"post",url:"processor/create/link",data:{id:$('#archive-id').val(),url:$('#archive-create-link-url').val(),name:$('#archive-create-link-name').val()},success:function(json){submitting_archive_create_link_form=false;if(json.code==200){// Refresh;window.location.reload()}else if(json.code==400){display_message([{'text':'Et element med denne URLen finnes fra før!','type':'danger'}]);$('#archive-create-link-form-submit').html('Lagre').prop('disabled',false)}else{display_message([{'text':'Noe gikk visst galt her!','type':'danger'}]);$('#archive-create-link-form-submit').html('Lagre').prop('disabled',false)}}})}}};return false});//// History//;if($('#archive-history').length>0){$.ajax({cache:false,type:"post",url:"processor/history/get",data:{id:$('#archive-id').val()},success:function(json){$('#archive-history-inside').html(json.html)}})}//// Alerts//;$('#main_messages').on('click','.alert-close',function(){// Remove;$(this).parent().fadeOut(400,function(){$(this).remove()})});if($('.alert').length>0){setTimeout(function(){// Close all messages;$('.alert-close').trigger('click')},10000)};//// New flags//;$('#archive-context-new-flag-name').on('click',function(e){e.preventDefault();// Title;$('#modal-new-flag .modal-title').html('Endre navn på: '+$archive_right_click.data('name'));// Set filetype;if($archive_right_click.data('type')=='dir'){$('#modal-new-flag-name .input-group .input-group-addon').html(' ')}else{var filetype_index=$archive_right_click.data('name').lastIndexOf('.');$('#modal-new-flag-name .input-group .input-group-addon').html($archive_right_click.data('name').substr(filetype_index))}// Show modal;$('#modal-new-flag').modal('show');// Hide all inners;$('.modal-new-flag-container').hide();$('#modal-new-flag-name').show()});$('#modal-new-flag-name-form').on('submit',function(){// Check if valid;if($('#modal-new-flag-name-name').val().length==0){alert('Du glemte å skrive inn et navn...')}else{$.ajax({cache:false,type:"post",url:"processor/flag/name",data:{id:$archive_right_click.data('id'),name:$('#modal-new-flag-name-name').val(),filetype:$('#modal-new-flag-name .input-group .input-group-addon').html(),comment:$('#modal-new-flag-name-comment').val()},success:function(json){if(json.code==200){// Refresh;window.location.reload()}else{alert('Noe gikk visst galt her!')}}})};return false});$('#archive-context-new-flag-delete').on('click',function(e){e.preventDefault();// Title;$('#modal-new-flag .modal-title').html('Sletting av: '+$archive_right_click.data('name'));// Show modal;$('#modal-new-flag').modal('show');// Hide all inners;$('.modal-new-flag-container').hide();$('#modal-new-flag-delete').show()});$('#modal-new-flag-delete-form').on('submit',function(){$.ajax({cache:false,type:"post",url:"processor/flag/delete",data:{id:$archive_right_click.data('id'),comment:$('#modal-new-flag-delete-comment').val()},success:function(json){if(json.code==200){// Refresh;window.location.reload()}else{alert('Noe gikk visst galt her!')}}});return false});//// Stuff//;$('#dropdown-menu-opener').on('click',function(e){setTimeout(function(){$('#login-email').focus()},100)});$('.login-opener').on('click',function(e){e.preventDefault();e.stopPropagation();$('#login-dropdown').dropdown('toggle');setTimeout(function(){$('#login-email').focus()},100)});//// Profile//;var profile_email_value="";$('#profile-edit-info-form input').on('keyup',function(){var $that=$(this);var $that_parent=$that.parent();var element_id=this.id;if(this.id=='register-form-email'){if(check_email($that.val())){if($that_parent.hasClass('has-error')){$that_parent.removeClass('has-error')};$('#register-form-email-error1').css('color','');if(profile_email_value!=$('#register-form-email').val()){profile_email_value=$('#register-form-email').val();$.ajax({cache:false,type:"post",url:"processor/register/check",data:{email:$('#register-form-email').val(),ignore:true},success:function(json){if(json.code==200){$('#register-form-email-error2').css('color','')}else{$('#register-form-email-error2').css('color','red')}}})}}else{if(!$that_parent.hasClass('has-error')){$that_parent.addClass('has-error')};$('#register-form-email-error1').css('color','red')}}});//// Grayboxes//;if($('#archive-sidebar-newest-inner').length>0){$.ajax({cache:false,url:'graybox/newest',success:function(html){// Set content;$('#archive-sidebar-newest-inner').html(html);// Load moment;$('#archive-sidebar-newest-inner .moment-timestamp').each(function(){$that=$(this);$that.html(moment($(this).data('ts')).fromNow())})}})};if($('#archive-sidebar-last-downloads-inner').length>0){$.ajax({cache:false,url:'graybox/downloads',success:function(html){// Set content;$('#archive-sidebar-last-downloads-inner').html(html);// Load moment;$('#archive-sidebar-last-downloads-inner .moment-timestamp').each(function(){$that=$(this);$that.html(moment($(this).data('ts')).fromNow())})}})};if($('#archive-sidebar-numbers-inner').length>0){$.ajax({cache:false,url:'graybox/numbers',success:function(html){// Set content;$('#archive-sidebar-numbers-inner').html(html)}})}//// Tooltips//;$('.list-group-item a').tooltip();//// Remove favorite from frontpage//;var removing_fav=false;$('.star-remove').on('click',function(){$el=$(this);if(!removing_fav){removing_fav=true;$.ajax({cache:false,type:"post",url:"processor/favorite/remove",data:{id:$el.data('id')},success:function(json){if(json.code==200){// Everything went better than expected:);$el.parent().slideUp(function(){$(this).remove();if($('#favorites-list li').length==0){$('#favorites-list').html('<li class="list-group-item" style="display: none;"><em>Du har ingen favoritter...</em></li>');$('#favorites-list li').slideDown()};removing_fav=false});// Display message;display_message(json.msg)}else{// Something went wrong;alert('Noe gikk visst galt her. Ups!');removing_fav=false}}})}});//// Debug//;$('#toggle-queries').on('click',function(e){e.preventDefault();var $that=$(this);if($('#queries').is(':visible')){$('#queries').slideUp(400,function(){$('span',$that).html('Vis')})}else{$('#queries').slideDown(400,function(){$('span',$that).html('Skjul')})}})})