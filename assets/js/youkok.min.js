...
$('#prefetch .typeahead,#prefetch2 .typeahead').typeahead({hint:true,highlight:true,},{name:'courses',displayKey:'course',source:courses.ttAdapter(),}).on('typeahead:selected',function($e,datum){check_check_search($e.target.id)});$('#s,#s2').on('keyup',function(e){if(e.keyCode==13){check_check_search(this.id)}});$('#nav-search,#nav-search2').on('click',function(){check_check_search($(this).parent().find('.tt-input').attr('id'))});var archive_id=0;var $archive_right_click=null;var $archive_context_menu=$('#archive-context-menu');var ignore_click_outside=false;if(site_data.user_online!=1){$('#archive-context-newflag-outer').hide();$('#archive-context-report').parent().hide()}
