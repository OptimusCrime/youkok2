...
return this._getXHRPromise(false,data&&data.context)}})}));;(function($,c,b){$.map("click dblclick mousemove mousedown mouseup mouseover mouseout change select submit keydown keypress keyup".split(" "),function(d){a(d)});a("focusin","focus"+b);a("focusout","blur"+b);$.addOutsideEvent=a;function a(g,e){e=e||g+b;var d=$(),h=g+"."+e+"-special-event";$.event.special[e]={setup:function(){d=d.add(this);if(d.length===1){$(c).bind(h,f)}},teardown:function(){d=d.not(this);if(d.length===0){$(c).unbind(h)}},add:function(i){var j=i.handler;i.handler=function(l,k){l.target=k;j.apply(this,arguments)}}};function f(i){$(d).each(function(){var j=$(this);if(this!==i.target&&!j.has(i.target).length){j.triggerHandler(e,[i.target])}})}}})(jQuery,document,"outside");;if(typeof String.prototype.endsWith!=='function'){String.prototype.endsWith=function(suffix){return this.indexOf(suffix,this.length-suffix.length)!==-1}}
...
function check_check_search(target){var val=$('#'+target).val();var datums=courses.index.datums;var datums_size=datums.length;var was_found=false;for(var i=0;i<datums_size;i++){var current_datum=datums[i];if(current_datum.course==val){was_found=true;window.location=$('#search-base').val()+current_datum.url;break}}
if(!was_found){$('#'+target).parent().parent().parent().submit()}}
$(document).ready(function(){moment.lang('en',{relativeTime:{future:"Om %s",past:"%s siden",s:"Noen få sekunder",m:"Ett minutt",mm:"%d minutter",h:"En time",hh:"%d timer",d:"en dag",dd:"%d dager",M:"En måned",MM:"%d måneder",y:"Et år",yy:"%d år"}});var courses=new Bloodhound({datumTokenizer:Bloodhound.tokenizers.obj.whitespace('course'),queryTokenizer:Bloodhound.tokenizers.whitespace,limit:10,prefetch:'processor/search/courses.json',});courses.initialize();if($('#typehead-cache-time').val()!=localStorage.getItem('ts')){localStorage.clear();localStorage.setItem('ts',$('#typehead-cache-time').val())}
$('#prefetch .typeahead,#prefetch2 .typeahead').typeahead({hint:true,highlight:true,},{name:'courses',displayKey:'course',source:courses.ttAdapter(),}).on('typeahead:selected',function($e,datum){check_check_search($e.target.id)});$('#s,#s2').on('keyup',function(e){if(e.keyCode==13){check_check_search(this.id)}});$('#nav-search,#nav-search2').on('click',function(){check_check_search($(this).parent().find('.tt-input').attr('id'))});var archive_id=0;var $archive_right_click=null;var $archive_context_menu=$('#archive-context-menu');var ignore_click_outside=false;if($('#archive-online').val()!=1){$('#archive-context-newflag-outer').hide();$('#archive-context-report').parent().hide()}
...
else{if($archive_context_menu.is(':visible')){$archive_context_menu.hide()}}});$('body').on('contextmenu','.archive-item',function(e){ignore_click_outside=true;var $that=$(this);$archive_right_click=$that;archive_id=$that.data('id');$('#archive-context-menu-id',$archive_context_menu).html($that.data('name'));$('#archive-context-menu-size',$archive_context_menu).html(' ('+human_file_size($that.data('size'),true)+')');$('#archive-context-menu-flags',$archive_context_menu).html($that.data('flags'));var link=$that.parent().attr('href');if($that.data('type')=='dir'){$('#archive-context-open',$archive_context_menu).show();$('#archive-context-open a',$archive_context_menu).show().attr('href',link);$('#archive-context-download,#archive-context-new-flag-delete,#archive-context-new-flag-move',$archive_context_menu).hide()}
else{$('#archive-context-download',$archive_context_menu).show();$('#archive-context-download a',$archive_context_menu).show().attr('href',link);$('#archive-context-download,#archive-context-new-flag-delete,#archive-context-new-flag-move',$archive_context_menu).show();$('#archive-context-open',$archive_context_menu).hide()}
if($('#archive-online').val()==1){if($that.data('favorite')==null){$('#archive-context-star',$archive_context_menu).hide()}
else{$('#archive-context-star',$archive_context_menu).show();if($that.data('favorite')==1){$('#archive-context-star-inside',$archive_context_menu).html('Fjern favoritt')}
else{$('#archive-context-star-inside',$archive_context_menu).html('Legg til favoritt')}}}
else{$('#archive-context-star',$archive_context_menu).hide()}
$archive_context_menu.css({display:'block',left:e.pageX-$('#archive-list').offset().left+45,top:e.pageY-$('#archive-top').offset().top+10,});return false});$('#archive-context-close').on('click',function(e){e.preventDefault();$archive_context_menu.hide()});$('#archive-context-flags,#archive-context-info,#archive-context-report').on('click',function(){$archive_context_menu.hide()});$('#archive-context-newflag').on('click',function(e){e.preventDefault()})
function load_flags(){$.ajax({cache:false,type:"post",url:"processor/flag/get",data:{id:archive_id},success:function(json){if(json.code==200){$('#flags-panel').html(json.html)}
...
$.ajax({cache:false,type:"post",url:"processor/favorite/"+favorite_type,data:{id:$that.data('archive-id')},success:function(json){if(json.code==200){if(json.status){$that.removeClass('archive-heading-star-0').addClass('archive-heading-star-1')}
...
else if(json.code==400){display_message([{'text':'Et element med dette navnet finnes fra før!','type':'danger'}])}
else{display_message([{'text':'Noe gikk visst galt her!','type':'danger'}])}}})}}
...
else{$('#forgotten-password-new-form-submit').prop('disabled',true)}});if($('#archive-list').length>0){var display=false;$('#archive-list li').each(function(){if(display==false&&$(this).find('div').data('type')=='file'){display=true}});if(display){$('#archive-zip').show()}}
$('#archive-create-file').on('click',function(){if($('#archive-create-file-div').is(':visible')){$('#archive-create-file-div').stop().slideUp()}
...
if(!found){for(var i=0;i<accepted_fileendings.length;i++){if(name.endsWith('.'+accepted_fileendings[i])){found=true;break}}}
...
else{var error=[{'text':'<p>Filtypen \'.'+this_filetype+'\' er ikke godkjent på Youkok2. Dersom du syntes denne filtypen burde godkjennes, kan du ta kontakt.</p>','type':'danger',},];display_message(error)}
